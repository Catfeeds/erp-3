...
class OnsaleController extends AdminbaseController {
    ...
    public function _initialize() {
        parent::_initialize();
        $this->Purchase = D("Common/WarehouseReturn");
   ...
   public function index(){
       ...
       $list = $purchase_model->order('id_return DESC')->select();

       foreach($list as &$row){
           $row['operation'] = in_array($row['status'], array(12, 11)) ? 'on_sale' : 'view';
   ...
   public function edit(){
       $id_return = I('request.id_return');
       $purchase_info = M('WarehouseReturn')->alias('p')
           ->field("p.*, w.title as warehouse_name")
           ->join("__WAREHOUSE__ as w ON w.id_warehouse=p.id_warehouse", "LEFT")
           ->where(array('id_return'=>$id_return))
           ->find();
       $product_info = M('WarehouseReturnProduct')->alias('pp')
           ...
           ->where(array('id_return'=> $id_return,'pp.type'=>1))
           ->select();
   ...
   public function save(){
       ...
       $product_info = M('WarehouseReturnProduct')->alias('pp')
           ...
           ->where(array('id_return'=> $post_data['id_return'],'type'=>1))
           ->select();

       $product_info = array_column($product_info, null, 'id_product_sku');
       $purchase_info = M('WarehouseReturn')->alias('p')
           ->where(array('id_return'=>$post_data['id_return']))
           ->find();
       ...
       foreach($sku_data as $value){
           ...
           if(!$exist){
               ...
               M('WarehouseAllocationStock')->where(array('id'=>array('EQ',$exist['id'])))->setField(array(
                   'updated_at' => date('Y-m-d H:i:s'),
                   'quantity' => $exist['quantity']+$value['quantity'],
                   'id_users' => $exist['id_users'] . ',' . $_SESSION['ADMIN_ID']
               ));
           }

           //添加产品库存
           M('WarehouseReturnProduct')->where(array(
               'id_return' => $purchase_info['id_return'],
               'id_product_sku' => $value['id_product_sku'],
           ))->setInc('quantity_on_sale', $value['num']);

           //采购单记录更新
           $purchase_info['total_onsale'] += $value['num'];
           if($purchase_info['total_onsale'] >= $purchase_info['total_received']){
               $status = PurchaseStatus::FINISH;
           }else{
               $status = PurchaseStatus::PART_ON_SALE;
           }
           M('WarehouseReturn')->where(array(
               'id_return' => $purchase_info['id_return']
           ))->save(array(
           ...
           D('WarehouseRecord')->write(
               array(
                   ...
                   'num' => $value['num'],
                   'id_product_sku' => $value['id_product_sku'],
                   'track_number' => $purchase_info['track_number']
               )
           );
    ...
    public function look(){
        ...
        $where['pp.id_return'] = $id_return;
        $where['pp.type'] = 1;
        $warehouse = M('Warehouse')->field('id_warehouse,title')->where('status=1')->select();
        $warehouse = array_column($warehouse,'title','id_warehouse');
        $purchase_product = M('WarehouseReturnProduct')->alias('pp')->field('pp.*,p.return_no,p.id_warehouse')->join('__WAREHOUSE_RETURN__ as p on p.id_return = pp.id_return','LEFT')->where($where)->select();
