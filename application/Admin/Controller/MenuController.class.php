...
class MenuController extends AdminbaseController {
    ...
    public function lists() {
        ...
        add_system_record($_SESSION['ADMIN_ID'], 4, 3, '查看后台所有菜单列表');
        ...
        if (IS_POST) {
            if ($this->menu_model->create() !== false) {
                if ($this->menu_model->add() !== false) {
                    ...
                    add_system_record($_SESSION['ADMIN_ID'], 1, 3, '添加菜单成功');
                    $this->success("添加成功！", U($to));
                } else {
                    add_system_record($_SESSION['ADMIN_ID'], 1, 3, '添加菜单失败');
                    $this->error("添加失败！");
                }
            } else {
    ...
    public function delete() {
        ...
        if ($this->menu_model->delete($id) !== false) {
            add_system_record($_SESSION['ADMIN_ID'], 3, 3, '删除菜单成功');
            $this->success("删除菜单成功！");
        } else {
            add_system_record($_SESSION['ADMIN_ID'], 3, 3, '删除菜单失败');
            $this->error("删除失败！");
        }
    }
        ...
        if (IS_POST) {
            if ($this->menu_model->create() !== false) {
                if ($this->menu_model->save() !== false) {
                    ...
                    $this->_export_app_menu_default_lang($app);
                    add_system_record($_SESSION['ADMIN_ID'], 2, 3, '编辑菜单成功');
                    $this->success("更新成功！");
                } else {
                    add_system_record($_SESSION['ADMIN_ID'], 2, 3, '编辑菜单成功');
                    $this->error("更新失败！");
    ...
    public function listorders() {
        ...
        if ($status) {
            add_system_record($_SESSION['ADMIN_ID'], 2, 3, '菜单排序成功');
            $this->success("排序更新成功！");
        } else {
            add_system_record($_SESSION['ADMIN_ID'], 2, 3, '菜单排序失败');
            $this->error("排序更新失败！");
        }
    }
    ...
    public function backup_menu() {
        ...
        add_system_record($_SESSION['ADMIN_ID'], 1, 3, '菜单备份成功');
        $this->success('菜单备份成功！');
    }
    ...
    public function export_menu_lang() {
        ...
        add_system_record($_SESSION['ADMIN_ID'], 2, 3, '生成菜单语言包');
    ...
    private function _import_menu($menus, $parentid = 0, &$error_menus = array()) {
        foreach ($menus as $menu) {
            ...
            add_system_record($_SESSION['ADMIN_ID'], 6, 3, '菜单导入');
        }
    }
    ...
    public function restore_menu() {
        ...
        if (empty($error_menus)) {
            add_system_record($_SESSION['ADMIN_ID'], 6, 3, '菜单恢复成功');
            $this->success('菜单恢复成功！');
        } else {
            add_system_record($_SESSION['ADMIN_ID'], 6, 3, '菜单恢复失败');
    ...
    public function add_menu_nav() {
        $menu = D("Common/Menu")->where(array('id' => array('GT', 177)))->order('id asc')->select();
        if ($menu) {
            foreach ($menu as $item) {
                ...
                $find_rule_count = $this->auth_rule_model->where($where)->count();
                print_r($find_rule_count);
                echo '<br />';
                if ($find_rule_count == 0) {
                    $this->auth_rule_model->add(array("name" => $name, "module" => $app, "type" => "admin_url", "title" => $menu_name)); //type 1-admin rule;2-user rule
                }
