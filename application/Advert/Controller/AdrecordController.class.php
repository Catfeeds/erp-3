 ...
 */
class AdrecordController extends AdminbaseController {
    ...
    private $type	= array(
        1=>array('name'=>'import_advert', 'cols'=>array('user_name','domain_name','advert_name','post_at','advert_status')),
        2=>array('name'=>'import_expense', 'cols'=>array('user_name','domain_name','conversion_at','expense','zone')),
    );
    /**
     * 导入广告
     */
    ...
    protected function import_list($type_id, $id=0) {
        ...
        $import_table_name 			= M('AdvertImport')->getTableName();
        $import_data_table_name 	= M('AdvertImportData')->getTableName();
        if($id>0)
        {
            $where['id_advert_import'] 	= array('EQ', $id);
        ...
        else
        {
            $count = $model->table($import_table_name . " t")
                ->join("{$import_data_table_name} ti ON (t.id = ti.id_advert_import)", 'LEFT')
            ...
            $list = $model->table($import_table_name . " t")
                ->join("{$import_data_table_name} ti ON (t.id = ti.id_advert_import)", 'LEFT')
                ->join("__USERS__ u ON (t.owner_id = u.id)", 'LEFT')
                ->field('t.*,ti.*,count(ti.id) as sum,u.user_nicename as owner_id_nicename,t.id as id_advert_import')
                ->where($where)
                ->group('t.id')
                ->order("t.id DESC")
                ->limit($page->firstRow, $page->listRows)
    ...
    private function detail($type_id) {
        ...
        if(!empty($id))
        {
            ...
            $import = M('AdvertImport')->where(array('id'=>$id))->find();
        }
    ...
    public function add_post() {
        ...
        if (IS_AJAX) {
            ...
            try
            {
                ...
                $id_advert_import	= I('post.id_advert_import');
                $save_data = array();
                $save_data['description'] 	= I('post.description');
                if($id_advert_import>0)
                {
                    M('AdvertImport')->where(array('id'=>array('EQ',$id_advert_import)))->save($save_data);
                }
                else
                {
                    ...
                    $id_advert_import = M('AdvertImport')->data($save_data)->add();
                }
                $message 	= '保存成功';
                $status 	= 1;
            ...
            $return = array('status' => $status, 'id' => $id_advert_import, 'message' => (is_array($message)?implode("\n",$message):$message));
...
//
                ...
                if(!empty($message))
                {
                    $return = array('status' => 0, 'id' => $id_advert_import, 'message' => (is_array($message)?implode("\n",$message):$message));
                    echo json_encode($return);exit();
                }
                ...
                if(!empty($data_tmp))
                {
                    ...
                    {
                        $v['id_advert_import'] = $id_advert_import;
                        M('AdvertImportData')->data($v)->add();
                    }
                }
    ...
    public function del_item() {
        if(IS_AJAX) {
            ...
            try {
                ...
                if(!empty($ids))
                {
                    M('AdvertImportData')->where(array('id'=>array('IN',$ids)))->delete();
                    $status	 = 1;
                    $message	 = '删除成功';
                }
    ...
    public function delete() {
        ...
        if(IS_AJAX) {
            try {
                ...
                if(!empty($id_advert_import))
                {

                    foreach($id_advert_import as $id)
                    {
                        M("AdvertImport")->where(array("id" => $id))->delete();
                        M("AdvertImportData")->where(array("id_advert_import" => $id))->delete();
                    }
        ...
        if (IS_AJAX) {
            ...
            try {
                ...
                if(!empty($id_advert_import))
                {
                    ...
                    {
                        $import 			= M('AdvertImport')->where(array('id'=>$id))->find();
                        $import_data 	= M("AdvertImportData")->where(array("id_advert_import" => $id))->select();
                        if(!empty($import))
                        {
                            ...
                            if(!empty($import_data))
                            {
                               $ret = $this->$submit($type_id,$import,$import_data);
                                if($ret){
                                    ...
                                    M('AdvertImport')->where(array('id'=>$id))->save($upd_data);
                                }
     ...
     * 提交（广告）
     */
    private function import_advert_submit($type_id,$import,$import_data)
    {
        $advertObj = M("Advert");
        ...
        foreach($import_data as $k => $v){
            ...
            $add['advert_name'] = $v['advert_name'];
            $add['created_at'] = date('Y-m-d H:i:s');
            $add['post_at'] = $v['post_at'];
            $add['advert_status'] = $v['advert_status'];
     ...
     * 提交（广告费用）
     */
    private function import_expense_submit($type_id,$import,$import_data)
    {
        $advertObj = M("Advert");
        $advertDataObj = M("AdvertData");
        foreach($import_data as $k => $v){
            $add['advert_id'] = $v['advert_id'];
            $add['expense'] = $v['expense'];
            $add['conversion_at'] = $v['conversion_at'];
            $add['created_at'] = $v['created_at'];
            $add['add_user'] = $v['add_user'];
            $add['id_zone'] = $v['id_zone'];
            $add['id_users_today'] = $v['id_users'];
    ...
    public function kkk(){
        if (IS_POST) {
            ...
            foreach ($data as $row) {
                ...
                $advert_name = trim($row[2], '\'" ');
                $post_at = trim($row[3], '\'" ');
                $advert_status = trim($row[4]);
//                $where_advert['user_nicename'] = $user_name;
                $where_advert['name'] = $domain_name;
                $getAdvert = $advertObj->alias('a')
                    ->field('advert_id,user_nicename')
                    ->join('__USERS__  as u on u.id = a.id_users ')
                    ->join('__DOMAIN__  as d on d.id_domain = a.id_domain ')
                    ->where($where_advert)->select();
                if($getAdvert){
                    foreach($getAdvert as $v){
                        $infor['error'][] = sprintf('第%s行:%s     已存在这条广告 %s', $count,$v['user_nicename'],$domain_name);
                    }
                }
                else{
                    ...
                    if($id_users&&$id_domain){
                        ...
                        $add['created_at'] = date('Y-m-d H:i:s');
                        $add['post_at'] = $post_at;
                        $add['advert_status'] = ($advert_status == '启用') ? 1 :($advert_status == '停用' ?  0 : '');
                        $add['add_user'] = $_SESSION['ADMIN_ID'];
                        $advertObj->add($add);
                        $infor['success'][] = sprintf('第%s行:添加广告成功 %s  %s  %s %s ', $count,$user_name,$domain_name,$advert_name,$post_at,$advert_status);
                    }else{
                        $infor['error'][] = sprintf('第%s行:优化师或者域名有误，添加失败 %s  %s  %s %s ', $count,$user_name,$domain_name,$advert_name,$post_at,$advert_status);
                    }
    ...
    public function lll($data){
        ...
        $where_advert['user_nicename'] = $user_name;
        $where_advert['name'] = $domain_name;
        $getAdvert = $advertObj->alias('a')->join('__USERS__  as u on u.id = a.id_users ','LEFT')
            ->join('__DOMAIN__  as d on d.id_domain = a.id_domain ','LEFT')
            ->where($where_advert)
            ->getField('advert_id');
        $id_users_today= M("Users")->where(['user_nicename'=>$user_name])->getField('id');
        $count=0;
        foreach($data as $v){
            $id_zone = M('Zone')->where(array('title'=>$zone))->getField('id_zone');
            if(!$id_zone){
                $infor['error'][] = sprintf('第%s行:找不到对应的地区 %s ', $count,$zone);
                $count++;
                continue;
            }
        }
        $add['advert_id'] = $getAdvert;
        $add['expense'] = $expense;
