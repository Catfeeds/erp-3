...
class IndexController extends AdminbaseController {
    ...
    public function index() {
        ...
        if (IS_POST) {
            ...
            if (I('post.mobile')) {
                $where['mobile'] = array('LIKE', '%' . I('post.mobile') . '%');
            }
            if (I('post.content')) {
                $where['content'] = array('LIKE', '%' . I('post.content') . '%');
            }
        }
        
        $count = $this->sms->where($where)->count();
        $page = $this->page($count, 20);

        $data = $this->sms->where($where)->order(array("id_sms" => "desc"))
        ...
        add_system_record(sp_get_current_admin_id(), 4, 3, '查看短信列表');
        $this->assign("sms", $data);
    ...
    public function add_post() {
        if (IS_POST) {
            ...
            if ($this->sms->create($data)) {
                if ($this->sms->add($data) !== false) {
                    add_system_record(sp_get_current_admin_id(), 1, 3, '添加短信成功');
                    $this->success("添加成功", U("Index/index"));
                } else {
                    add_system_record(sp_get_current_admin_id(), 1, 3, '添加短信失败');
                    $this->error("添加失败！");
                }
            } else {
                $this->error($this->sms->getError());
            }
        }
    }
    ...
    public function delete() {
        ...
        $status = $this->sms->delete($id);
        if ($status !== false) {
            add_system_record(sp_get_current_admin_id(), 3, 3, '删除短信'.$id.'成功');
            $this->success("删除成功！", U('Index/index'));
        } else {
            add_system_record(sp_get_current_admin_id(), 3, 3, '删除短信'.$id.'失败');
            $this->error("删除失败！");
        }
    }
    ...
    public function edit() {
        ...
        $data = $this->sms->where(array("id_sms" => $id))->find();
        if (!$data) {
            $this->error("该短信不存在！");
        }
    ...
    public function edit_post() {
        ...
        if (IS_POST) {
            if ($this->sms->create()) {
                if ($this->sms->save() !== false) {
                    add_system_record(sp_get_current_admin_id(), 2, 3, '修改短信'.$id.'成功');
                    $this->success("修改成功！", U('Index/index'));
                } else {
                    add_system_record(sp_get_current_admin_id(), 2, 3, '修改短信'.$id.'失败');
                    $this->error("修改失败！");
                }
            } else {
                $this->error($this->sms->getError());
...
//                exit;
    ...
    private function send_mail($array, $lock_file)
    {
        ...
        try {
            ...
            add_system_record(sp_get_current_admin_id(), 2, 3, '手动发送邮箱');            
