...
class HeimaoController extends AppframeController {
...
//            file_put_contents(CACHE_PATH.$this->lock_write_data, 'lock');
            ...
            //$where['os.id_shipping'] = array('exp',' IN (17,21,22,24,25,26,27,28,43,74)');
            //$where['os.track_number'] = array('exp','is not null');
            //$where['o.id_order_status'] = array('Not In','4,5,7,16,22');

            $time = date('Y-m-d 00:00:00',strtotime('-41 day'));
            $where['o.date_delivery'] = array('GT', $time);
            $where['o.id_order_status'] = 9;
            $where['os.id_shipping'] = 21;
            //$where['ost.status'] = 0;

            $data =  M('Order')->alias('o')
                ->field("o.*,os.track_number")
                ->join("__ORDER_SHIPPING__ AS os ON os.id_order=o.id_order", 'LEFT')
                //->join("__ORDER_SETTLEMENT__ AS ost ON os.id_order = ost.id_order" , 'LEFT')
                ->where($where)
                ->order('os.fetch_count desc')
                //->limit($i,100)
            ...
            file_put_contents("./logs/heimao",date('Y-m-d H:i:s'). "\n",FILE_APPEND);
    ...
    protected function result_handler($url, $value){
        ...
        echo "\r\n";
        file_put_contents("./logs/heimao",$value .' '. $status . ' ' . $sign_time . "\n",FILE_APPEND);
