...
class AdminbaseController extends AppframeController {
	public function __construct() {
		//国外服务器 跳转到国内服务器
		if($_SERVER['SERVER_NAME']=='www.hepxi.com' or $_SERVER['SERVER_NAME']=='hepxi.com' or $_SERVER['SERVER_NAME']=='23.230.237.110'){
	...
	function _initialize(){
        ...
        //获取最后登录的IP ，如果根目前操作的IP不同跳转到登录页面
//        $get_ip = get_client_ip(0,true);
//        $allow_ip = F('user_allow_ip');
//        $allow_ip = $allow_ip?json_decode($allow_ip,true):array();
//        $difference = time()-session('last_login_time');
//        if($difference>28800 && !in_array($get_ip,$allow_ip) && $get_ip !=session('last_login_ip')){
//            //header("Location:".U("admin/public/login"));
//            unset($_SESSION['ADMIN_ID']);session('ADMIN_ID',0);
//            $this->error('您的IP已经变更，请重新登录', U("admin/public/login"));
//            exit();
//        }
		...
		/*if(isset($_SESSION['last_access']) && time()-$_SESSION['last_access']>1800)
		{
			//此时应该设置session失效
			unset($_SESSION['ADMIN_ID']);session('ADMIN_ID',0);
			session('last_access',time());//退出后重新设置过期时间
			$this->error('30分钟无操作请求，请重新登录', U("admin/public/login"));
			exit();
		} else {
	...
	private function check_access($uid){
		...
		$no_need_check_rules=array(
            "AdminIndexindex","AdminMainindex",'PurchaseIndexget_attr','PurchaseIndexget_invalid','WarehouseOrdercancel_order','OrderIndexget_attr_html',
            'SettlementOrderanalysisget_parities','SettlementOrderanalysisget_count_price','WarehouseWaveprint_number','WarehouseWaveget_waybill',
            'ProductIndexget_category','ProductIndexget_three_category','PurchaseIndexget_supp_attr','PurchaseIndexget_product_title','WarehouseWaveget_shipping_attr',
            'ProductCheckextendtime','ProductCheckper_ret','ProductCheckhide','PurchaseIndexget_supp_url','PurchaseIndexget_supplier_title','GoodslocationCargochannelget_goods_location',
            'GoodslocationAreaselect_by_warehouse','GoodslocationAreaselect_by_goods_area_coding','WarehouseChangesearch_sku','WarehouseChangeajax_get_sku','ProductTransfersearch_product',
            'ProductTransferajax_get_pro','ProductTransfersetpagerow','WarehouseChangesetpagerow','WarehouseChangeadd_submit','ProductChecksearch_text','ProductCheckajax_category','WarehouseWaveupdate_order_status',
			'ProductChecksearch_cat_text','ProductChangechange_product_model','WarehouseImportupdate_package_code_save'
        );
