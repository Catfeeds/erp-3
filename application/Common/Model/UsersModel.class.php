...
class UsersModel extends CommonModel
{
    ...
    //递归查找所有下级
    public function get_all_subordinate($user_id, $subordinates = array()){
        if(is_array($user_id)){
            $this->where(array('superior_user_id'=>array('IN', $user_id)));
        }else{
            $this->where(array('superior_user_id'=>array('EQ', $user_id)));
        }
        $subordinate = $this->field('id')->select();
        if(!empty($subordinate)){
            $temp_subordinate = array_column($subordinate, 'id');
            $subordinates = array_merge($subordinates, $temp_subordinate);
            return $this->get_all_subordinate($temp_subordinate, $subordinates);
        }else{
            return $subordinates;
        }
    }
