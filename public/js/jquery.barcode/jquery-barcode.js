...
function generateBarcode(config){
    var set_value   = config.set_value?config.set_value:'empty';
    var btype = config.btype?config.btype:'code128';
    var renderer = config.renderer?'canvas':'css';
    var quietZone = false;
    var settings = {
        showHRI:config.showHRI?false:true,
        output:renderer,
        bgColor: '#FFFFFF',
        color: '#000000',
        barWidth: config.barWidth?config.barWidth:'1',
        barHeight: config.barHeight?config.barHeight:'50',
        fontSize: 15,
        moduleSize: 5,
        posX:10,
        posY: 20,
    ...
    set_value = {code:set_value, rect: true};
    var set_div = config.set_div?config.set_div:'#barcodeTarget';
    $(set_div).html("").show().barcode(set_value, btype, settings);
};