...
function session($name='',$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
        ...
        if(C('SESSION_AUTO_START')){
            $current_session = session_get_cookie_params();
            session_set_cookie_params($current_session['lifetime'], $current_session['path'],$current_session['domain'], $current_session['secure'], true);
            session_start();
        }
    }elseif('' === $value){ 
        if(''===$name){
